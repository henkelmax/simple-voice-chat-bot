...
public class SupportCommand {
    ...
    private static void onSupportCommand(SlashCommandCreateEvent event) {
        ...
        event.getSlashCommandInteraction().respondLater().thenAccept(responseUpdater -> {
            channel.getMessages(16).thenAccept(messages -> {
                ...
                if (optionalMessage.isPresent()) {
                    ...
                    builder.setDescription("This is not a support channel! Please open a support ticket in <#%s>.".formatted(supportChannel.getId()));
